...
graph = pydot.Dot(graph_type='graph', strict=True, bgcolor="#fff3af",
...
def number_of_cannibals_exceeds(number_missionaries, number_cannibals):
    ...
    return (number_missionaries > 0 and number_cannibals > number_missionaries) or \
        (number_missionaries_right > 0 and number_cannibals_right > number_missionaries_right)
...
def generate():
    ...
    q = deque()
    node_num = 0
    q.append((3, 3, 1, 0, node_num))  # (missionaries, cannibals, side, depth, node number)
    ...
    while q:
        ...
        # Draw edge from u -> v
        # Where u = Parent[v]
        # and v = (number_missionaries, number_cannibals, side, depth_level, node_num)
        u, v = draw_edge(number_missionaries, number_cannibals, side, depth_level, node_num)   #loi o day
        ...
        for x, y in options:
            ...
            if Parent[(number_missionaries, number_cannibals, side, depth_level, node_num)] is None or \
                    (next_m, next_c, next_s) != Parent[(
            number_missionaries, number_cannibals, side, depth_level, node_num)][:3]:
