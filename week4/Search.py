    ...
    while not queue.empty():
        ...
        if node not in closed:
            ...
            for neighbor in graph.can_see(node):
                ...
                if neighbor not in closed or new_cost < neighbor.g:
                    neighbor.g = new_cost
                    neighbor.pre = node
...
def visualize_path(graph, poly_list, start, goal, result, algorithm_name, subplot_idx):
    ...
    for i in range(1, len(poly_list) - 1):
        ...
        coord.append(coord[0])  # Close the polygon
        xs, ys = zip(*coord)
        plt.plot(xs, ys)

    # Plot the path
    path_x = [point.x for point in result]
    path_y = [point.y for point in result]
    plt.plot(path_x, path_y, 'b', linewidth=2.0)
...
def main():
    ...
    with open('Input.txt', 'r') as f:
        ...
        algorithms = {
            'A*': lambda: search(graph, start, goal, a_star),
            'Greedy': lambda: search(graph, start, goal, greedy),
        ...
        for idx, (algorithm_name, algorithm) in enumerate(algorithms.items(), 1):
            # Get the path
            result_node = algorithm()
            ...
            print(f"{algorithm_name} path: ", *print_res, sep=' -> ')

            # Visualize the path
            visualize_path(graph, poly_list, start, goal, result, algorithm_name, idx)
